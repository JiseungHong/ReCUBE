...
from utils.agent.cce_open_models.apptainer_env_graph import ApptainerEnvironmentManager
from utils.agent.cce_open_models.agent_wrapper_graph_apptainer import GraphMiniSWEAgentRunner
...
def run_inference_for_repo(
    ...
    apptainer_manager = ApptainerEnvironmentManager(base_image="docker://python:3.12")

    # Create Apptainer environment with graph tools
    try:
        container_name = apptainer_manager.create_environment(
            repo_id=repo_id,
        ...
        print(f"Error creating Apptainer environment: {e}")
        return {
            'repo_id': repo_id,
            'status': 'error',
            'error': str(e),
    ...
    for i, target_file in enumerate(target_files, 1):
        ...
        apptainer_manager.prepare_target_file(repo_id, target_file)
        ...
        apptainer_manager.restore_original_file(repo_id, target_file)

        # Small delay between files
        time.sleep(2)

    # Cleanup Apptainer environment
    print("\nCleaning up Apptainer environment...")
    apptainer_manager.cleanup(repo_id)
