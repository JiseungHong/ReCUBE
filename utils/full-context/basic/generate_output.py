#!/usr/bin/env python3
"""
Inference script for LLM code reconstruction task (Target files only).
...
4. Prompting the LLM to reconstruct the file based on context
5. Saving the reconstructed outputs
...
def create_prompt(modified_instruction: str, target_file: str) -> str:
    """
    Create the prompt for the LLM to reconstruct the file.
...
You must respond with ONLY the reconstructed file content wrapped in a code block as follows:

```python
# Your complete reconstructed file here
# Include all imports
# Include all function implementations
```

Do not include any explanations or additional text outside the code block.
The code block must contain the complete, runnable Python file with all imports and implementations.
...
def extract_code_from_response(response: str) -> tuple[str, bool]:
    """
    Extract the Python code from LLM response.
    Assumes the code is in a ```python ... ``` block.

    Returns:
        tuple[str, bool]: (extracted_code, has_correct_format)
    """
    ...
    # If no code block found, clean up any stray markdown markers and return the response
    # (LLM might not have used code block formatting, or included markers as content)
    cleaned_response = response.strip()

    # Remove leading markdown markers if present
    if cleaned_response.startswith('```python'):
        cleaned_response = cleaned_response[9:].lstrip('\n')
    elif cleaned_response.startswith('```'):
        cleaned_response = cleaned_response[3:].lstrip('\n')

    # Remove trailing markdown marker if present
...
def run_llm_inference(prompt: str, api_key: str, model: str, cost_tracker: CostTracker, max_retries: int = 3) -> str:
    """
    ...
    Returns:
        LLM response text
    """
    # Use OpenAI's default API endpoint (api.openai.com)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run LLM inference for code reconstruction (target files only)')
    ...
    # Output directory structure: outputs/full-context_basic/{model}/{repo_id}/
    OUTPUTS_BASE_DIR = Path("outputs/full-context_basic") / MODEL
    ...
    for target_entry in target_data:
        ...
        # Create output directory for this instruction: outputs/Code/{model}/{repo_id}/
        ...
        for i, target_file in enumerate(selected_files, 1):
            ...
            for retry in range(3):
                try:
                    llm_response = run_llm_inference(prompt, api_key, MODEL, cost_tracker)
            ...
            reconstructed_code, has_correct_format = extract_code_from_response(llm_response)
            ...
            print(f"  {format_indicator} Saved to {output_file.name}")
        ...
        print(f"\n✓ Inference completed for repo {repo_id}")
        ...
        print(f"    - Correct format (```python```): {inference_status['format_stats']['correct_format']}")
